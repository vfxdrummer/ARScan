...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, UIDocumentPickerDelegate {
    ...
    var detectionObjects: [ARReferenceObject] = []
    ...
    // MARK: - AR Reference Object Loading
    
    private func loadARReferenceObject() {
        // set referenceObjectToTest
        // call testObjectDetection()
        
        guard let detectionObjects = ARReferenceObject.referenceObjects(inGroupNamed: "Pacifico2", bundle: nil) else {
            fatalError("Missing expected asset catalog resources.")
        }
        referenceObjectToTest = detectionObjects.first
        testObjectDetection(with: detectionObjects)
        self.state = .testing
    }
    ...
    @IBAction func addScanButtonTapped(_ sender: Any) {
        ...
        alertController.addAction(UIAlertAction(title: "Merge New Scan…", style: .default) { _ in
            ...
            self.referenceObjectToMerge = self.testRun?.referenceObjects.first
            self.state = .startARSession
        })
    ...
    func testObjectDetection() {
        ...
        if let object = referenceObjectToTest {
            testObjectDetection(with: [object])
            return
        }
        ...
        scan.createReferenceObject { scannedObject in
            if let object = scannedObject {
                self.testObjectDetection(with: [object])
            } else {
    ...
    func testObjectDetection(with objects: Set<ARReferenceObject>) {
        self.testRun?.setReferenceObjects(objects, screenshot: scan?.screenshot)
    ...
    func createAndShareReferenceObject() {
        guard let testRun = self.testRun, let object = testRun.referenceObjects.first, let name = object.name else {
...
//                state = .scanning
    ...
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        if let objectAnchor = anchor as? ARObjectAnchor {
            if let testRun = self.testRun, objectAnchor.referenceObject == testRun.referenceObjects.first {
    ...
    fileprivate func mergeIntoCurrentScan(referenceObject: ARReferenceObject, from url: URL) {
        if self.state == .testing {
            ...
            self.testRun?.referenceObjects.first?.mergeInBackground(with: referenceObject, completion: { (mergedObject, error) in
                let alertController = UIAlertController(title: nil, message: nil, preferredStyle: .alert)
                
                if let mergedObject = mergedObject {
                    mergedObject.name = self.testRun?.referenceObjects.first?.name
                    self.testRun?.setReferenceObjects([mergedObject], screenshot: nil)
                    ...
                    let otherScan = UIAlertAction(title: "Use Other Scan", style: .default) { _ in
                        self.testRun?.setReferenceObjects([referenceObject], screenshot: nil)
                    }
